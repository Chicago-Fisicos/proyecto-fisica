...
PATH_TABLA_MOV_METROS = '../pelotas-chocando/tablas/tabla-moviento-metros-basket-despues-de-choque.csv'
CANT_FRAMES = 58.85167464114833
MEASUREMENT_ERROR_METROS = 0.01
PIXEL_METROS = 0.00441
...
def main():
    ...
    plt.title('Energía Cinética y Potencial - Basquet despues del Choque')
    plt.legend()
    plt.grid(True)

    # Guardar el gráfico
    plt.savefig('energias_cinetica_potencial_basquet_despues.png')
    ...
    tabla_de_Errores['Gravedad Promedio - Error'] = g_prom - error_gravedad
    tabla_de_Errores.round(2).to_csv('Tabla_de_Errores_basket_despues_choque.csv', index=False)
