...
def main():
    ...
    plt.title('Energía Cinética y Potencial - Tenis antes del Choque')
    plt.legend()
    plt.grid(True)

    # Guardar el gráfico
    plt.savefig('energias_cinetica_potencial_tenis_antes.png')
    ...
    tabla_de_Errores['Gravedad Promedio - Error'] = g_prom - error_gravedad
    tabla_de_Errores.round(2).to_csv('Tabla_de_Errores_tenis_antes_choque.csv', index=False)
